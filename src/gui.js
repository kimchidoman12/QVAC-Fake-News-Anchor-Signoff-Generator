#!/usr/bin/env node
// QVAC Fake News Anchor Signoff Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29428;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/anchorsignoff") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake News Anchor Signoff Generator GUI ready at http://localhost:${PORT}`);
  });
